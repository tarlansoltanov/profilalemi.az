(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    define("/worker/", ["jquery", "Site"], factory);
    ...
    global.workers_index = mod.exports;
  }
  ...
  (function () {
    (0, _jquery.default)(document).ready(function () {
      ...
      $('#workers thead tr:eq(1) th').each(function () {
        var title = $(this).text();
        if (title != '') {
      ...
      var options = _jquery.default.extend(true, {}, defaults, {
        ...
        initComplete: function () {
          ...
          $("#workers_filter").append(column_search_btn);
          ...
          $("#workers_filter").append(clear_search_btn);
          clear_search_btn.button();
          clear_search_btn.on('click', function () {
            $('#workers_filter input').val('');
            table.search('').draw();
      ...
      $('#workers').dataTable(options);

      setInterval(function () {
        $('#searchHeader th').each(function () {
          var cell_display = $('#workers thead tr:eq(0) th:eq(' + $(this).index() + ')').css('display');
